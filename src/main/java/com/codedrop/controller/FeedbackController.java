    ...
    public ResponseEntity<?> getAll(@RequestParam(required = false) String query) {
        if (query == null) {
            ...
            try {
                ...
                Page<Feedback> paginatedUsers = feedbackService.findPaginateWithConditions(page, size, conditions);
                CustomPage<Feedback> customPage = new CustomPage<>(paginatedUsers);
